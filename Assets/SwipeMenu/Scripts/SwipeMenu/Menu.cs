...
namespace SwipeMenu
{
	...
	public class Menu : MonoBehaviour
	{
		...
		{
			float offset = CalcPosXInverse (item.transform.localPosition.x);
		...
		private MenuItem GetClosestMenuItem ()
		{
			...
			foreach (var i in menuItems) {
				var x = CalculateOffsetFromX (i.gameObject.transform.localPosition.x, 0);
		...
		private int GetClosestMenuItemIndex ()
		{
			...
			for (int i = 0; i < menuItems.Length; i++) {
				var x = CalculateOffsetFromX (menuItems [i].gameObject.transform.localPosition.x, 0);
